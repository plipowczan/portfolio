...
4. **Five tests have never run.** `seo-security-headers` (4) and `perf-font-cache-headers` (1) both gate on `SEO_HEADERS_URL`, which nothing sets — not locally, not in CI, not in the merge gate. They are the only automated coverage of `vercel.json`. (Counted by running them, not by grepping: the earlier figure of seven counted each file's `test.skip()` guard as a test.)
