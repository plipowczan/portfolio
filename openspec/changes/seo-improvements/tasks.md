...
- [ ] 1.1 Przenieść logikę injection `clickrank.ai` w `index.html` do funkcji `inject()` opakowanej w `requestIdleCallback(inject, { timeout: 3000 })` z fallbackiem `setTimeout(inject, 2000)` (feature detect `'requestIdleCallback' in window`).
...
- [ ] 2.1 Przygotować `public/logo-schema.png` o wymiarach 600×60 (konwersja z `public/logo.svg`, np. `sharp` lub online converter; weryfikacja ostrości).
- [ ] 2.2 Dodać stałą `SITE_CONFIG.schemaLogo = "/logo-schema.png"` do `src/utils/constants.js`.

## 3. Schema enrichment (BlogPosting)

- [ ] 3.1 Rozszerzyć walidację frontmatter w `src/data/blogPosts.js` o opcjonalne pola `description` (string) i `modified` (YYYY-MM-DD). Nie są `required`.
- [ ] 3.2 Dodać util `src/utils/extractFirstParagraph.js` — wyciąga pierwszy akapit z markdown (pomija nagłówki i frontmatter), trimuje do ~300 znaków na granicy słowa, dodaje `…` jeśli obcięte.
- [ ] 3.3 W `src/pages/BlogPostPage.jsx` rozszerzyć `blogPostingSchema`:
...
- [ ] 3.4 Zaktualizować 1-2 posty testowe dodając frontmatter `description` i `modified`, zweryfikować że fallback działa dla postów bez tych pól.
...
- [ ] 4.1 W `src/components/seo/SEO.jsx` dodać prop `modifiedTime`. Emitować `<meta property="article:modified_time" content={modifiedTime}>` gdy `article && modifiedTime`.
- [ ] 4.2 W `src/pages/BlogPostPage.jsx` przekazać `modifiedTime={post.modified || post.date}` do `<SEO>`.

## 5. GEO — llms.txt + llms-full.txt

- [ ] 5.1 Stworzyć `scripts/generate-llms-txt.js`:
...
- [ ] 5.2 Wpiąć generator do `scripts/build-with-prerender.mjs` przed krokiem vite build (lub równolegle).
- [ ] 5.3 Zadecydować o commitowaniu plików: commitować (łatwiejszy review, mały koszt) — upewnić się że nie są w `.gitignore`.
...
- [ ] 6.1 Założyć konto na report-uri.com (free tier), wygenerować endpoint dla CSP reports, zanotować URL.
- [ ] 6.2 W `vercel.json` w sekcji `headers` dla `/(.*)`:
...
- [ ] 6.5 Pozostawić notatkę w `design.md` że enforcing CSP to osobny change po 2-4 tygodniach analizy raportów.

## 7. Sitemap — per-post lastmod

- [ ] 7.1 Przebudować `scripts/update-sitemap.js`:
...
- [ ] 7.2 Uruchomić `npm run blog:sitemap` lokalnie, zweryfikować że `public/sitemap.xml`:
...
- [ ] 8.1 Uruchomić pełny `npm test` (Playwright E2E) — zero regresji.
- [ ] 8.2 Dodać Playwright test: `public/llms.txt` zwraca 200 i zawiera pattern `# Pawel Lipowczan`.
- [ ] 8.3 Dodać Playwright test: `public/llms-full.txt` zwraca 200 i zawiera treść przynajmniej jednego znanego posta.
- [ ] 8.4 Dodać Playwright test: response headers na `/` zawierają `Referrer-Policy`, `Strict-Transport-Security`, `Permissions-Policy`, `Content-Security-Policy-Report-Only` i NIE zawierają `X-XSS-Protection`.
...
- [ ] 9.1 Zaktualizować `.claude/rules/data-storage/00-overview.md` — dodać pola `description` i `modified` do opisanego frontmatter schema (oznaczyć jako opcjonalne).
- [ ] 9.2 Zaktualizować `docs/blog/BLOG_WORKFLOW.md` (jeśli istnieje) o nowe opcjonalne pola.
