...
- [~] 6.3 Run `PW_ALL=1 PW_PREVIEW=1 npm test`; verify the full matrix passes
  - **Not run as written, and deliberately.** `tests/AGENTS.md` states that nothing manual gates a merge: the pull request runs `chromium` + `Mobile Chrome`, the push to `main` runs the full matrix, and full-matrix failures are fixed forward on `main` by design. A local six-project run also cannot finish — `globalTimeout` is 20 minutes outside CI, and the machine kills the run for memory first: one attempt died after 302 green tests with no failures.
  - **What did run**, on `chromium` and `Mobile Chrome`, the same pair that gates the pull request: the mapped set for every touched path (`a11y-baseline`, `testimonials`, `home`, `contact-form`, `booking-cta`, `ui-ux-audit`, `llm-wiki-landing`, `llm-wiki-course`, `blog`, `breadcrumbs`) — 319 passed, 0 failed; and the remainder (`analytics-consent`, `llm-wiki-discoverable`, `hydration-paint`, `policy-pages`, `projects`, `language-switcher-blog`, `perf-image-loading`, `seo-llms-txt`, `perf-self-hosted-fonts`) — 65 passed, 0 failed, 18 skipped for want of `PW_PREVIEW=1`.
  - The full matrix runs on the push to `main`, per the rule above.
- [x] 6.4 Run the DOX pass: `tests/AGENTS.md` gains the new spec and its project mapping, and `.claude/rules/playwright/30-testing.md` gains its row in the change-to-test table, or state explicitly that a doc is unchanged
