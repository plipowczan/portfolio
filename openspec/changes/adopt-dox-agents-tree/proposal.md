...
**Modified as a consequence of the tree**
- `src/data/blogPosts.js`, `src/data/coursePosts.js`, `scripts/update-sitemap.js`, `scripts/prerender.mjs`, `scripts/generate-llms-txt.js` — each globs a content folder for `*.md` and previously excluded only `README.md`. Dropping `AGENTS.md` and `CLAUDE.md` into `src/content/blog/` and `src/content/kurs/` turned them into phantom articles and lessons, and broke `npm run blog:sitemap` outright with `Invalid time value`. All five now filter through a shared `DOC_FILES` set.
- `.claude/skills/blog-article-writer/subcommands/{prime,plan,translate}.md` and `.claude/commands/blog-article-writer/{plan,translate}.md` — the same exclusion, because these list or grep `src/content/blog/*.md` and the docs are usually the newest files there.
- `docs/README.md` — indexes the new `PROJECT_STATUS.md` and the existing `TODO.md`.
...
- Application behaviour. The loader and script edits above are filename filters only; `public/llms.txt` and `public/llms-full.txt` regenerate byte-identical, and the prerender output is unchanged at 98 pages.
