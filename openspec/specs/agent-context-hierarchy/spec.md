# agent-context-hierarchy Specification

## Purpose
TBD - created by archiving change adopt-dox-agents-tree. Update Purpose after archive.
## Requirements
...
### Requirement: Content-globbing readers exclude the tree's own docs

`src/content/blog/` and `src/content/kurs/` are read as content by globbing `*.md`. Placing an `AGENTS.md` and a `CLAUDE.md` in them makes the documentation indistinguishable from an article or a lesson at the glob level.

Every reader of a content folder SHALL exclude `README.md`, `AGENTS.md`, and `CLAUDE.md` by filename. This applies to `src/data/blogPosts.js`, `src/data/coursePosts.js`, `scripts/update-sitemap.js`, `scripts/prerender.mjs`, and `scripts/generate-llms-txt.js`. Adding an `AGENTS.md` to any further globbed folder SHALL include the corresponding exclusion in the same change.

Commands in `.claude/skills/blog-article-writer/` and `.claude/commands/blog-article-writer/` that list or grep `src/content/blog/*.md` SHALL apply the same exclusion, because the docs are typically the most recently modified files in the folder and the frontmatter schema example contains a literal `id:` line.

#### Scenario: Sitemap generation survives the tree

- **WHEN** `npm run blog:sitemap` runs with `AGENTS.md` and `CLAUDE.md` present in `src/content/blog/` and `src/content/kurs/`
- **THEN** it completes successfully and emits no URL derived from a documentation file

#### Scenario: Prerender emits no phantom route

- **WHEN** `npm run build:prerender` runs
- **THEN** no route is generated for `AGENTS.md` or `CLAUDE.md`, and the page count matches the number of real articles, lessons, projects, and static pages

#### Scenario: Next blog ID ignores the schema example

- **WHEN** the article workflow greps `src/content/blog/*.md` for `^id:` to find the next free ID
- **THEN** the `id:` line inside the frontmatter example in `src/content/blog/AGENTS.md` is excluded from the result
