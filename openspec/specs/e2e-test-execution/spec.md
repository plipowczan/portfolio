# e2e-test-execution Specification

## Purpose

Governs **when and where** the Playwright suite runs — not what it asserts.

A local run is scoped to the change at hand; a pull request gets a narrow but fast gate; `main` gets the full browser matrix; a finished deployment gets the checks that only a deployment can answer. The driving constraint is that a feedback loop nobody waits for is a feedback loop nobody uses: before this, every local verification cost tens of minutes at full CPU regardless of what changed.
...
The preview server (`build:test` + `preview:test`) SHALL start only under an explicit opt-in environment variable, never on `CI` alone. Tests that require it SHALL skip with a message naming the variable when it is absent, rather than fail. CI SHALL set the variable in the jobs whose selected tests need it.

Playwright starts every configured `webServer` regardless of test selection, so an unconditional preview server charges a full production build to every run — including runs of a single unrelated spec. Making it an explicit variable rather than an implicit consequence of `CI` keeps that decision visible per job, and reversible when the set of preview-dependent tests changes.
...
- **AND** the preview-dependent blocks report as skipped with a message naming the opt-in variable

#### Scenario: Run with the preview opt-in

- **WHEN** the suite runs with the preview opt-in set
- **THEN** the preview server starts
- **AND** the preview-dependent blocks execute their assertions

#### Scenario: CI jobs

- **WHEN** CI runs any browser job
- **THEN** it sets the opt-in variable, because a preview-dependent block — the consent gate, which proxies the production hostname to that build — runs on every project, not only on `chromium`
...
The bar is strict, and `testIgnore` works per file: a single assertion about visibility, layout, focus, animation or viewport keeps the whole file on the full matrix.
...
Security and cache headers come from `vercel.json`, so neither the dev server nor `vite preview` emits them. These suites are the only automated coverage of that file, and without a trigger that supplies the URL they skip in every environment.
