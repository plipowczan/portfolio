...
Dodaj <script type="application/ld+json"> z FAQPage schema przez komponent StructuredData Head component. Schema JSON zawiera @type: "FAQPage" oraz mainEntity array z Question objects. Każdy Question ma name (pytanie) i acceptedAnswer (Answer object z text). Możesz użyć helper function do auto-generowania schema z markdown FAQ content.
