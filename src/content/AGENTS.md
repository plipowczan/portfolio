...
- Every Polish text in this subtree follows
  ...
  folder doc.
- Frontmatter is a contract, not metadata. It is parsed and validated at build
  time by `scripts/generate-content.mjs`, and a file that does not hold up fails
  the build, naming the file and the problem.
- A `slug` is a live URL. Renaming one requires a redirect.

## Work Guidance

- Markdown is parsed with `gray-matter` **at build time** and rendered with
  `react-markdown` (`src/components/ui/MarkdownContent.jsx`). Neither the
  markdown bodies nor `gray-matter` reach the browser — see `src/data/AGENTS.md`.
